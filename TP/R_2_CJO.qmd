...
Ce package n'étant pas sur le CRAN, pour les installer on peut par exemple utiliser la fonction `remotes::install_github()` ou le dépot `rjdverse.r-universe.dev` (ou `aqlt.r-universe.dev` pour les versions en developpement):
```{r, eval=FALSE}
...
2. Calendrier associé au Sénégal, la difficulté étant qu'il faut ajouter à la main des jours associés aux fêtes musulmanes
...
# jours_mobiles <- readxl::read_excel("DataCVS/DateFetesMusulmanes_Senegal.xlsx") |>
#   as.data.frame()
...
# cat(sprintf(
#   'single_day("%s"), # %s',
#   all_dates$date, all_dates$holiday),
#   sep= "\n")
...
::: callout-note
## Exercice
Comparer le régresseurs JO *working days* créé avec le calendrier sénégalais et celui sans hypothèse sur les jours fériés (fonction `td()`).
:::
...
# Utilisation des régresseurs dans la V3 {#sec-rjd2}

Dans la V3 de RJDemetra, pour utiliser nos régresseurs jours ouvrables personnalisés, il faut :
...
Une autre façon de faire est d'ajouter le calendrier du Sénégal comme calendrier par défaut, ce qui permet de garder les options de JDemetra+ comme le préajustement de l'effet *leap-year* ou la sélection automatique des jeux de régresseurs.
